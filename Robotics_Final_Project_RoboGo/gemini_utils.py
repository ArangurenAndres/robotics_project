...
# Prompt used for the vision systems to describe the scene.
GEMINI_SCENE_PROMPT = """
...
# Template prompt for navigation tasks, filled in with the specific goal object.
...
def encode_frame_to_base64(frame_bgr) -> str:
    # Convert BGR frame to RGB and encode as JPEG base64.
    try:
        rgb_frame = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
    except cv2.error as e:
        rgb_frame = frame_bgr   # Fallback if conversion fails
...
def ask_gemini(model, b64_img: str, prompt: str) -> str:
    # Send an image and prompt to Gemini and get the text response.
...
def parse_navigation_advice(advice_text: str) -> dict:
    # Parse structured navigation advice text into a dictionary.
    parsed = {
